...
// Mirrors the slash-command default. The public default does not need a Gateway auth
// bypass; this artifact remains for explicitly configured legacy `/api/**` callbacks.
const DEFAULT_SLASH_CALLBACK_PATH = "/mattermost/command";
...
function isMattermostBypassPath(path: string): boolean {
  return path.startsWith("/api/channels/mattermost/");
}
