    ...
    expect(
      MattermostConfigSchema.safeParse({
        permalinkHydration: {
          ...
          maxPosts: 50,
          maxChars: 20000,
          allowedOrigins: ["https://mattermost.example.com"],
        },
      }).success,
    ).toBe(true);
    expect(
      MattermostConfigSchema.safeParse({ permalinkHydration: { maxLinks: 6 } }).success,
    ).toBe(false);
    expect(
      MattermostConfigSchema.safeParse({ permalinkHydration: { maxChars: 999 } }).success,
    ).toBe(false);
    expect(
      MattermostConfigSchema.safeParse({
        permalinkHydration: { allowedOrigins: ["https://mattermost.example.com/path"] },
      }).success,
    ).toBe(false);
  });
