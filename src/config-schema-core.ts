...
const MattermostPermalinkOriginSchema = z.string().refine(
  ...
  { message: "expected an HTTP(S) origin without path, credentials, query, or fragment" },
);

const MattermostPermalinkHydrationSchema = z
  .object({
    enabled: z.boolean().optional(),
    maxLinks: z.number().int().min(1).max(5).optional(),
    maxPosts: z.number().int().min(1).max(100).optional(),
    maxChars: z.number().int().min(1000).max(60000).optional(),
    /** Public reverse-proxy origins accepted as aliases for this Mattermost instance. */
    allowedOrigins: z.array(MattermostPermalinkOriginSchema).max(10).optional(),
  })
  .strict()
  .optional();
...
const MattermostAccountSchemaBase = z
  .object({
    ...
    permalinkHydration: MattermostPermalinkHydrationSchema,
