      ...
      return await new Promise<void>((resolve, reject) => {
        ...
        ws.on("message", async (data) => {
          ...
          try {
            ...
            // Mattermost cannot nack or replay a failed WebSocket event. Tear the
            // connection down loudly so the outage is operator-visible instead of
            // silently dropping every subsequent post against a broken handler.
            opts.runtime.error?.(
              `mattermost posted event handling failed; terminating websocket: ${String(err)}`,
            );
            ws.terminate();
          }
        });
