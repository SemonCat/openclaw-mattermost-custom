...
export function isConfiguredMattermostReadTarget(params: {
  cfg: OpenClawConfig;
...
export function isMattermostThreadTargetAllowed(params: {
  cfg: OpenClawConfig;
  account: ReturnType<typeof resolveMattermostAccount>;
  currentChannelId: string;
  targetChannel: MattermostChannel;
}): boolean {
  if (params.targetChannel.id === params.currentChannelId) {
    return true;
  }
  // Never hydrate a different DM, group DM, or private channel into the current
  // conversation. Cross-channel reads are limited to configured public channels.
  if (params.targetChannel.type !== "O") {
    return false;
  }
  return isConfiguredMattermostReadTarget({
    cfg: params.cfg,
    account: params.account,
    channelId: params.targetChannel.id,
  });
}
