...
export function createMattermostClient(params: {
  ...
  const request = async <T>(path: string, init?: MattermostRequestInit): Promise<T> => {
    ...
    if (path === "/reactions" && init?.method?.toUpperCase() === "POST") {
      ...
      // Reaction creation is a no-result mutation; its caller discards the receipt.
      return undefined as T;
    }
...
export async function fetchMattermostUsersByIds(
  ...
  const response = await client.request<unknown>("/users/ids", {
    method: "POST",
    body: JSON.stringify(userIds),
  });
  const parsed = z.array(MattermostUserSchema).safeParse(response);
  if (!parsed.success) {
    throw new Error("Unexpected Mattermost users response.");
  }
  return parsed.data;
}
...
export async function fetchMattermostPostThread(
  ...
  if (options.limit !== undefined && (!Number.isSafeInteger(options.limit) || options.limit <= 0)) {
    throw new Error("Mattermost thread limit must be a positive integer.");
  }
  const limit = Math.min(options.limit ?? 50, 100);
  const query = new URLSearchParams({ perPage: String(limit + 1) });
  const response = await client.request<unknown>(
    `/posts/${encodeURIComponent(rootPostId)}/thread?${query.toString()}`,
  );
  ...
  if (!parsed.success || parsed.data.order.some((postId) => !parsed.data.posts[postId])) {
    throw new Error("Unexpected Mattermost post thread response.");
  }
  const messages = parsed.data.order
    .slice(0, limit)
    .map((postId) => parsed.data.posts[postId] as MattermostPost);
  return {
    messages,
    hasMore:
      parsed.data.order.length > limit ||
      Boolean(parsed.data.next_post_id) ||
      Boolean(parsed.data.prev_post_id),
  };
}
...
export async function fetchMattermostFileInfo(
  ...
  const response = await client.request<unknown>(`/files/${encodeURIComponent(fileId)}/info`);
  const parsed = MattermostFileInfoSchema.safeParse(response);
  if (!parsed.success) {
    throw new Error("Unexpected Mattermost file info response.");
  }
  return parsed.data;
}
